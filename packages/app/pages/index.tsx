...
import { Connector, useAccount, useConnect, useContractRead, useContractWrite, useNetwork, useProvider, useWaitForTransaction } from "wagmi";
...
const Home: NextPage = () => {
    ...
    const getOwnerAddress = (ISO: string): string | null => {
        return getContinentId(ISO) != -1 && contractData ? contractData[getContinentId(ISO)][1] : null;
    }
    ...
    return accountLoading || networkLoading || contractLoading ? <>Loading...</> : (
        <>
            ...
            />
            {connectData.connected && contractData && <MapChart setContinent={setContinent} setIsOpen={setIsOpen} onTooltipChange={handleContentChange} contractData={contractData} accountData={accountData} readContractData={readContinents} />}
            ...
            <Modal
                ...
                style={modalStyle}
            >
                <div className="flex place-content-end"><Button onClick={() => setIsOpen(false)}>close</Button></div>
                <ContinentInfo continentSelected={continentSelected} />
            ...
            <Notifications errors={[connectError, networkError, accountError, contractError, priceError, relinquishContinentError, transferContinentError, acquireContinentError, transactionError]} />
        </>
    );
}
