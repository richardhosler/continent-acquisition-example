...
contract ContinentToken is ERC721 {
    ...
    function acquireContinent(bytes2 ISO) public payable {
        ...
        require(!strCompare(_continents[ISO].name, ""), string(abi.encodePacked("continent ",ISO," doesn't exist")));
    ...
    function allContinentsStatus() external view returns(Continent[7] memory){
        return [_continents["AF"], _continents["AS"], _continents["EU"], _continents["NA"], _continents["SA"], _continents["OC"], _continents["AN"]];
    }
    function continentStatus(bytes2 ISO) external view returns(Continent memory){
        return _continents[ISO];
    }
    function transferContinent(address from, address to, bytes2 ISO) public {
        require(msg.sender == _continents[ISO].owner);
